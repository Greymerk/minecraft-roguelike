...
public class ItemSupply extends ItemBase{
	...
	public ItemSupply(int weight, Difficulty diff) {
		...
		loot.add(new WeightedRandomLoot(Items.RABBIT_FOOT, 1, 1, 1));
		...
		loot.add(new WeightedRandomLoot(Items.GUNPOWDER, 8, 64, 1));
		
		
		WeightedRandomizer<ItemStack> saplings = new WeightedRandomizer<ItemStack>(1);
		FlowerPot.saplings.forEach(sapling -> {
			saplings.add(new WeightedRandomLoot(FlowerPot.getFlowerItem(sapling).getItem(), 1, 1, 1));
		});
		loot.add(saplings);
	}
